...
### `--force-config` сбросил мой кастомный AGH admin port

**Причина:** известное ограничение — `--force-config` пересевает
`bind_port: 3000` (см. `ROADMAP.md §Known limitations`).

**Что делать:** пройти wizard заново и снова выставить свой порт. Либо
не использовать `--force-config` если уже настроено.
